...
- **앱 실행에는 API 키가 필요 없습니다.** 기본으로 **전국 화장실 47,779곳**
  실데이터가 포함돼 있습니다 (공공데이터 + OSM + 제천 개방화장실 병합).
  (데이터가 없으면 서울 목업으로 폴백)
...
- `구분명`(공중/개방/간이/이동)과 `화장실소유구분명`(공공기관/민간)을 **함께** 봐서
  **개방화장실 + 민간 소유**일 때만 `type: "open"` 을 준다.
  앱의 "🏬 개방" 배지는 이용자에게 "남의 가게·건물 화장실이라 영업시간에 묶이고
  양해를 구해야 할 수 있다"는 뜻으로 읽히는데, `구분명`은 법적 분류라
  구청 소유 공원 화장실도 '개방화장실'로 등록되기 때문이다.
  (성동구는 214곳 전부를 개방화장실로 등록했지만 그중 150곳이 공공 소유다.
   전국으로도 개방화장실 22,214곳 중 민간 소유는 5,852곳뿐이다.)
...
전국 4.8만 건을 한 파일로 두면 12MB 라 앱을 켤 때마다 통째로 파싱해야 한다.
그래서 **0.5° 격자(한 변 약 50km)로 쪼개** `src/data/regions/` 에 저장한다.

- 타일 91개, 가장 큰 타일 977KB(서울), 중앙값 55KB
- 함께 생성되는 `index.ts` 는 Metro 가 정적으로 분석할 수 있는 `() => require()` 맵이라,
  각 타일은 **처음 필요할 때 비로소 파싱된다**
- [`src/api/toilets.ts`](src/api/toilets.ts) 가 사용자 좌표로 타일을 고른다.
  타일 경계에서 3km 안쪽이면 이웃 타일도 함께 읽어, 경계에 서 있어도
  전량 검색과 같은 결과가 나온다 (6개 지점으로 검증)

> ⚠️ 이건 **시작 시 파싱량**을 줄이는 것이지 앱 다운로드 용량을 줄이지는 않는다.
> Metro 는 정적으로 require 된 JSON 을 모두 번들에 넣기 때문에 번들은 여전히 ~10MB 다.
> 다운로드 용량까지 줄이려면 일부 지역만 번들하고 나머지는 원격에서 받아야 한다.
...
│   ├── data/
│   │   └── regions/        # 0.5° 격자 타일 + index.ts (파이프라인 생성)
│   ├── components/
